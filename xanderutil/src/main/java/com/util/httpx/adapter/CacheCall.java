...
import com.util.core.xhttp.JJHttp;
import com.util.core.xhttp.cache.CacheEntity;
import com.util.core.xhttp.cache.CacheManager;
import com.util.core.xhttp.cache.CacheMode;
import com.util.core.xhttp.callback.AbsCallback;
import com.util.core.xhttp.callback.AbsCallbackWrapper;
import com.util.core.xhttp.exception.JJhttpException;
import com.util.core.xhttp.model.HttpHeaders;
import com.util.core.xhttp.model.Response;
import com.util.core.xhttp.request.BaseRequest;
import com.util.core.xhttp.utils.HeaderParser;
...
public class CacheCall<T> implements Call<T> {
        ...
        if (cacheMode == CacheMode.IF_NONE_CACHE_REQUEST) {
            ...
            if (cacheEntity != null && !cacheEntity.isExpire()) {
                ...
                if (data == null || headers == null) {
                    //由于没有序列化等原因,可能导致数据为空
                    sendFailResultCallback(true, rawCall, null, JJhttpException.INSTANCE("没有获取到缓存,或者缓存已经过期!"));
                } else {
        ...
        rawCall.enqueue(new okhttp3.Callback() {
            ...
            public void onResponse(okhttp3.Call call, okhttp3.Response response) throws IOException {
                ...
                if (responseCode == 304 && cacheMode == CacheMode.DEFAULT) {
                    if (cacheEntity == null) {
                        sendFailResultCallback(true, call, response, JJhttpException.INSTANCE("服务器响应码304，但是客户端没有缓存！"));
                    } else {
                        T data = cacheEntity.getData();
                        HttpHeaders headers = cacheEntity.getResponseHeaders();
                        if (data == null || headers == null) {
                            //由于没有序列化等原因,可能导致数据为空
                            sendFailResultCallback(true, call, response, JJhttpException.INSTANCE("没有获取到缓存,或者缓存已经过期!"));
                        } else {
                ...
                if (responseCode == 404 || responseCode >= 500) {
                    sendFailResultCallback(false, call, response, JJhttpException.INSTANCE("服务器数据异常!"));
                    return;
                }
    ...
    private void sendFailResultCallback(final boolean isFromCache, final okhttp3.Call call, final okhttp3.Response response, final Exception e) {
        ...
        JJHttp.getInstance().getDelivery().post(new Runnable() {
            @Override
            public void run() {
                if (isFromCache) {
